...
class PhoneCheck
{
     ...
     * @var string
     */
    static public $hiddenUnicode = "/[\x{007f}-\x{009f}]|\x{00ad}|[\x{0483}-\x{0489}]|[\x{0559}-\x{055a}]|\x{058a}|
    [\x{0591}-\x{05bd}]|\x{05bf}|[\x{05c1}-\x{05c2}]|[\x{05c4}-\x{05c7}]|[\x{0606}-\x{060a}]|[\x{063b}-\x{063f}]|
    \x{0674}|[\x{06e5}-\x{06e6}]|\x{070f}|[\x{076e}-\x{077f}]|\x{0a51}|\x{0a75}|\x{0b44}|[\x{0b62}-\x{0b63}]|
    [\x{0c62}-\x{0c63}]|[\x{0ce2}-\x{0ce3}]|[\x{0d62}-\x{0d63}]|\x{135f}|[\x{200b}-\x{200f}]|[\x{2028}-\x{202e}]|
    \x{2044}|\x{2071}|[\x{f701}-\x{f70e}]|[\x{f710}-\x{f71a}]|\x{fb1e}|[\x{fc5e}-\x{fc62}]|\x{feff}|\x{fffc}/u";
    ...
    /**
     * 港澳联系方式，00852/00853/852/853+8位数字
     *
     * @var string
     */
    static public $telPattern00 = '/(?<phone>(?:^(00)?85[23]\d{8}))/';
     ...
     * @var string
     */
    static public $telPattern03 = '/(?<phone>(?:^0[^012]\d{2}[-|\s]?\d{7,8}([-|\s]?\d{1,4})?))/';
    ...
    static public function analysis(string $str, string $city = ''):array
    {
        ...
        if (strlen($str) < 18) {
            ...
            if ($match) {
                ...
                $firstC = current($ret);
                if (1 == \count($ret) && 1 == $firstC) {
                    $sep = array_search($firstC, $ret);
                } else {
...
//            logWarning('请输入正确的联系方式!', self::$logKey, $str);
    ...
    static public function match(string $str, string $city = ''):array
    {
        ...
        } elseif (\in_array($threeC, ['400', '800'], true)) {
            $phone = self::hasCorp($str);
            if ($phone) {
                $data['tel'] = self::formatCorp($phone);
            }
        } elseif ('0' === $firstC || (\in_array($threeC, ['852', '853']) && \strlen($str) > 8)) {
    ...
    {
        $pattern00 = self::$telPattern00;
        preg_match($pattern00, $str, $matches00);
        if (isset($matches00['phone']) && $matches00['phone']) {
            return $matches00['phone'];
        }
    ...
    static private function formatTel(string $tel):string
    {
        ...
        $code = substr($tel, 0, $codeN);
        $numberN = self::$telRules[$code] ?? 0;
        if (!\in_array($numberN, [7, 8])) {
//            logWarning($code . ' not exist', self::$logKey, $tel);
            throw new \InvalidArgumentException('不存在此电话区号，请核对！', 101303);
        }
        $number = substr($tel, $codeN, $numberN);
        $ret = $code . '-' . $number;
        $codeNumber = $codeN + $numberN;

        if (strlen($tel) < $codeNumber) {
            $msg = '区号' . $code . '下固话是' . $numberN . '位，' . $tel . '不符合规则！';
//            logWarning($msg, self::$logKey, $tel);
            throw new \InvalidArgumentException($msg, 101304);
        }

        if (strlen($tel) > $codeNumber) {
            $ret .= '-' . substr($tel, $codeNumber);
        }
        return $ret;
    }

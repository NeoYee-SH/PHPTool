...
1. 指定区号后面座机号的位数
```php
$telRules = ['010'  => 8,'021'  => 8,'022'  => 8,'023'  => 8,'024'  => 8,'025'  => 8,'027'  => 8,
...
```
2. 指定市级单位的区号(支持别名、简称、全称)

```php
$cityCode = ['上海市' => '021','上海' => '021','杭州市' => '0571','杭州' => '0571','南京市' => '025','南京' => '025','宁波市' => '0574','宁波' => '0574','温州市' => '0577','温州' => '0577','嘉兴市' => '0573','嘉兴' => '0573','湖州市' => '0572','湖州' => '0572','绍兴市' => '0575','绍兴' => '0575','金华市' => '0579','金华' => '0579','衢州市' => '0570','衢州' => '0570','舟山市' => '0580',
...
#### 坑
1. 使用空格分割符， 同时手机号中间还用空格分隔开(交给产品去解决)
```
$str = "021 88679089 134 4258 7758";
```
识别结果
```
Array
(
    [tel] => 021-88679089-1344
    [mobile] => 
)
```
如果同时提供两种联系方式建议用分号或者冒号分割。

2. 从iphone通讯录中复制过来带不可见的unicode字符(先过滤，增加了一次计算成本)
```
$str = "(0376) 803 9551;13478936245‬";
try
{
    $ret = PhoneCheck::analysis($str);
...
print_r($ret);
--
手机号和固话之前请用单个逗号或者分号分隔！

```

所以先进行了一次不可见字符的过滤， 这个过程可以优化， 有必要的话请自行改进
```
...
3. 联系方式中带有全角字符会报错(再加一次全角符号转成半角的操作)
```php
function dbc2Sbc(string $str):string {
    $dbc = array(
        '－' , '　' , '：' , '．' , '，' , '／' , '％' , '＃' , '！' , '＠' ,
        '＆' , '（' , '）' , '＜' , '＞' , '＂' , '＇' , '？' , '［' , '］' ,
        '｛' , '｝' , '＼' , '｜' , '＋' , '＝' , '＿' , '＾' , '￥' , '￣' ,
...
#### Demo1 带区号的固话在前，与手机号通过非空非数字字符隔开
```php
